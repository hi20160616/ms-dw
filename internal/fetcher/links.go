...
func fetchLinks() ([]string, error) {
	...
	for _, rawurl := range configs.Data.MS["dw"].URL {
	...
	rt = linksFilter(rt, `.*?\/zh\/.*`)
	rt = kickOutLinksMatchPath(rt, "s-")
	rt = kickOutLinksMatchPath(rt, "index-zh")
	rt = kickOutLinksMatchPath(rt, "av-")
	return rt, nil
}
...
func getLinks(rawurl string) ([]string, error) {
	...
	if links, err := exhtml.ExtractLinks(u.String()); err != nil {
		return nil, errors.WithMessagef(err, "[%s] cannot extract links from %s",
			configs.Data.MS["dw"].Title, rawurl)
	} else {
